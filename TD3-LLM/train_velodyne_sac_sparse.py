...
file_name = "SAC_velodyne_LLM_1000_4o"  # name of the file to store the policy
...
while timestep < max_timesteps:
    ...
    if done:
        ...
        if episode_num != 0:
            ...
            if not target and episode_timesteps > 20 and np.random.uniform(0, 1) < sample_rate and use_LLM_HER:
                ...
                if len(odom_state_sequence) > 100:
                    odom_state_sequence = odom_state_sequence[:100]
                ...
                if end_index-start_index > 9:
                    ...
                    reward_scale = np.clip(1 - distance/8, 0.8, 1)
                    writer.add_scalar('train/goal_distance', distance, episode_num)
                    old_state, old_action, old_reward, old_done_bool, old_done, old_next_state, odom_x, odom_y, angle, goal_x, goal_y = replay_buffer.get_last_episode(start_index)
                    new_state, _, _ = env.calculate_reward_and_state(old_state, odom_x, odom_y, angle, new_goal_x, new_goal_y)
                    for i in range(start_index+1, end_index):
                        ...
                        replay_buffer.add(new_state, old_action, new_reward * reward_scale, new_done_bool, new_done, new_next_state, odom_x, odom_y, angle, new_goal_x, new_goal_y)
