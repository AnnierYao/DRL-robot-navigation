...
file_name = "ddpg_velodyne_LLM"  # name of the file to store the policy
            ...
            if not target and episode_timesteps>1 and np.random.uniform(0, 1) < 0.5 and episode_num < 200 and use_LLM_HER:
                ...
                if new_goal_x != info['goal_x'] or new_goal_y != info['goal_y']:
                    ...
                    for i in range(episode_timesteps):
                        ...
                        replay_buffer.add(new_state, old_action, new_reward, new_done_bool, new_done, new_next_state, odom_x, odom_y, angle, goal_x, goal_y)
                        if new_done:
                            break
